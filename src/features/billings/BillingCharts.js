...
const BillingCharts = () => {
  ...
  useEffect(() => {
    const generateColors = (length) => {
      return Array.from({length}, (_, i) => CHART_COLORS[i % CHART_COLORS.length]);
    };
    setColors(generateColors(data.length));
  }, [data.length]);

  // Memoized processed data
  const lineChartData = useMemo(() => processDataForLineChart(data), [data, processDataForLineChart]);
  const barChartData = useMemo(() => processDataForBarChart(data), [data, processDataForBarChart]);
  const pieChartData = useMemo(() => processDataForPieChart(data), [data, processDataForPieChart]);
  const areaChartData = useMemo(() => processDataForAreaChart(data), [data, processDataForAreaChart]);
  const scatterChartData = useMemo(() => processDataForScatterChart(data), [data, processDataForScatterChart]);
  const averageBillingAmountData = useMemo(() => processDataForAverageBillingAmount(data), [data, processDataForAverageBillingAmount]);
              ...
              <ResponsiveContainer width="100%" height={400}>
                <BarChart data={barChartData}>
                  ...
                  <ChartTooltip cursor
                                content={<ChartTooltipContent hideLabel/>}/>
                  <Legend/>
                  <Bar dataKey="billingAmount" fill={CHART_COLORS[0]}
                       radius={4}/>
                </BarChart>
              </ResponsiveContainer>
            </ChartContainer>
            ...
            <ChartContainer config={chartConfig}>
              <ResponsiveContainer width="100%" height={400}>
                <ScatterChart data={scatterChartData}>
                  <CartesianGrid/>
                  <XAxis type="number" dataKey="billingAmount"
                         name="billing amount"/>
                  <YAxis type="number" dataKey="totalReceived"
                         name="total received"/>
                  <ZAxis range={[100]}/>
                  <ChartTooltip cursor
                                content={<ChartTooltipContent hideLabel/>}/>
