...
const AppointmentsToday = dynamic(() => import('@/features/patient/PAppointments/AppointmentsToday'), {
  loading: () => <LoadingPage />,
});
const PatientsCharts = dynamic(() => import('@/features/patients/PatientsCharts'), {
  loading: () => <LoadingPage />,
});

export default function Home() {
  const { data: Appointments = [], isPending: isLoadingAppointments, error: appointmentsError } = useFetchAppointments();
  const { data: Patients = [], isPending: isLoadingPatients, error: patientsError } = useFetchPatients();
  ...
  if (hasError) {
    return <ErrorComponent message="Error loading data. Please try again later." />;
  }
